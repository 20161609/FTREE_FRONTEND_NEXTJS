...
import { api_test, api_get_user_info } from '@/libs/api_user'; // Example API functions
import ModalForgetPassword from '@/components/ModalForgetPassword';

export default function LoginPage() {
  const router = useRouter(); // Moved router inside the client-side effect
  const [credentials, setCredentials] = useState({ useremail: '', password: '' });
  const [isModalOpen, setModalOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    // This effect will only run in the client-side
